...
class ContentSortModel extends Model
{
    ...
    // 批量更新文章时间
    public function batchUpdateTime($ids, $new_time)
    {
        // 将文章ID转为逗号分隔的字符串
        $ids_str = implode(',', $ids);
    
        // 执行批量更新操作
        $query = "UPDATE `ay_content` SET `update_time` = ? WHERE `id` IN ($ids_str) AND `acode` = ?";
        $stmt = $this->db->prepare($query);
        
        // 执行SQL语句，更新指定ID的文章时间
        return $stmt->execute([$new_time, session('acode')]);
    }
    ...
    public function getSubSortRows($scode)
    {
        $this->scodes = array();
        $this->getSubScodes($scode);
        $scodes = array_diff($this->scodes, array($scode));
        if (! $scodes) {
            return array();
        }
        return parent::table('ay_content_sort')->field('scode,mcode,select_filter_values,select_filter_manual_values,select_filter_sync')
            ->in('scode', $scodes)
            ->where("acode='" . session('acode') . "'")
            ->select();
    }

    public function getExtFieldValue($mcode, $field)
    {
        return parent::table('ay_extfield')->where("mcode='$mcode'")->where("name='$field'")->value('value');
    }
