...
class ModelModel extends Model
{
    ...
    // 规范化扩展表名
    public function normalizeExtTableName($tname)
    {
        $tname = trim($tname);
        if ($tname === '') {
            return '';
    ...
    // 获取模型扩展表名
    public function getExtTableName($mcode)
    {
        $tname = parent::table('ay_model')->where("mcode='$mcode'")->value('tname');
        $tname = $this->normalizeExtTableName($tname);
        if (! $tname || $tname === false) {
    ...
    // 创建独立扩展表
    public function ensureExtTable($tname)
    {
        $tname = $this->normalizeExtTableName($tname);
        if (! $tname || $tname === 'ay_content_ext') {
            return true;
        }

        $db = get_db_type();
        if ($db == 'mysql') {
            return $this->amd("CREATE TABLE IF NOT EXISTS `$tname` LIKE `ay_content_ext`");
        } elseif ($db == 'sqlite') {
            $sql = parent::table('sqlite_master')
                ->where("type='table' AND name='ay_content_ext'")
                ->value('sql');
            if ($sql) {
                $create = preg_replace('/^CREATE TABLE/i', 'CREATE TABLE IF NOT EXISTS', $sql);
                $create = preg_replace('/CREATE TABLE\s+[`"\[]?ay_content_ext[`"\]]?/i', 'CREATE TABLE IF NOT EXISTS ' . $tname, $create, 1);
                return $this->amd($create);
            }
        } elseif ($db == 'pgsql') {
            return $this->amd("CREATE TABLE IF NOT EXISTS $tname (LIKE ay_content_ext INCLUDING ALL)");
        }
