layui.use(['element','upload','laydate','form'], function(){
  ...
  //跳转
	...
	form.on('select(contentScode)', function(data){
		if (typeof window.refreshContentSelectFilterOptions === 'function') {
			window.refreshContentSelectFilterOptions($(data.elem).parents('form')[0]);
		}
	});
	...
	,done: function(res, index){
	   if(res.code==1){
		   ...
		   html += "<dl><dt><img src='"+sitedir+res.data[0]+"' data-url='"+res.data[0]+"'></dt><dd>删除</dd>" +
		   		"<dt><input type='text' name='picstitle[]' value='"+filessname[index]+"' style='width:95%;border:solid 1px #ddd;padding-top:5px;padding-bottom:5px;' /></dt>"+		
		   		"</dl>";
  ...
  //选择模型切换模板
   form.on('select(model)', function(data){
	  ...
	  addOptionToSelect($form.find('select[name="listtpl"]'), listtpl, listtpl);
	  addOptionToSelect($form.find('select[name="contenttpl"]'), contenttpl, contenttpl);
	  $form.find('select[name="listtpl"]').val(listtpl || '');
	  $form.find('select[name="contenttpl"]').val(contenttpl || '');
	  if (typeof window.refreshSelectFilterBuilder === 'function') {
		  window.refreshSelectFilterBuilder($form[0], true);
	  }
