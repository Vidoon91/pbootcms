...
class MemberCommentController extends Controller
{
    ...
    // 生成评论对应的前台内容链接
    private function parserContentLink($comment)
    {
        if (! $comment) {
            return '';
        }
        if (! empty($comment->outlink)) {
            return $comment->outlink;
        }

        $url_break_char = $this->config('url_break_char') ?: '_';
        $url_rule_content_path = $this->config('url_rule_content_path') ? true : false;
        $urlname = $comment->urlname ?: 'list';
        $sortfilename = $comment->sortfilename;
        $contentfilename = $comment->filename;
        $scode = $comment->scode;
        $id = $comment->contentid;

        if ($url_rule_content_path) {
            $link = $contentfilename ? Url::home($contentfilename, true) : Url::home($id, true);
        } else {
        ...
        if ($link && strpos($link, 'http') !== 0) {
            $link = rtrim(get_http_url(), '/') . '/' . ltrim($link, '/');
        }
        return $link;
    }
    ...
    public function index()
    {
        if ((! ! $id = get('id', 'int')) && $result = $this->model->getComment($id)) {
            $result->contentlink = $this->parserContentLink($result);
