...
class ModelController extends Controller
{
    ...
    public function add()
    {
        if ($_POST) {
            ...
            $tname = $this->model->normalizeExtTableName($tname_raw);
            if ($tname === false) {
                alert_back('独立表名格式错误，只允许字母、数字、下划线！');
            }
            if ($tname === 'ay_content_ext') {
                $tname = '';
            }
            if ($tname) {
                $this->model->ensureExtTable($tname);
            }
