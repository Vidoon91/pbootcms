...
class ConfigController extends Controller
{
    ...
    public function index()
    {
        ...
        if (empty($configs['select_url_path']['value'])) {
            $configs['select_url_path']['value'] = '1';
        }
        if (empty($configs['select_url_path_rules']['value'])) {
            $configs['select_url_path_rules']['value'] = $this->getDefaultSelectUrlPathRules();
        }
        ...
        $this->assign('content_models', model('admin.content.Model')->getSelect());
        $this->assignSelectUrlPathFields();
        
        $this->display('system/config.html');
    }

    private function assignSelectUrlPathFields()
    {
        ...
        $this->assign('select_url_path_fields_json', htmlspecialchars($json, ENT_QUOTES, 'UTF-8'));
    }
    ...
    private function modDbConfig($key)
    {
        ...
        if ($key == 'select_url_path_rules') {
            $value = isset($_POST[$key]) ? trim($_POST[$key]) : '';
            $value = html_entity_decode(stripcslashes($value), ENT_QUOTES, 'UTF-8');
            if ($value) {
                $json = json_decode($value, true);
                if (! is_array($json) || ! isset($json['models']) || ! is_array($json['models'])) {
                    alert_back('筛选路径配置 JSON 格式错误，请使用 {"models":{"模型编号":["字段名"]}} 格式！');
                }
                if (isset($json['fields'])) {
                    unset($json['fields']);
                }
                $value = json_encode($json, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
            }
            $this->model->ensureValueText();
            if ($this->model->checkConfig("name='$key'")) {
                return $this->model->modTextValue($key, $value);
            }
            $data = array(
                'name' => $key,
        ...
        if ($key == 'home_upload_ext') {
            // 不允许特殊扩展
            if (preg_match('/(php|jsp|asp|sh|cmd|vb|vbs|phtml)/i', $value)) {
                return;
            }
        }
    ...
    private function getDefaultSelectUrlPathRules()
    {
