...
class IndexController extends Controller
{
    ...
    public function _empty()
    {
        ...
        if ($this->parser->selectPathUrlEnabled() && count($path_arr) > 1) {
            for ($i = count($path_arr) - 1; $i >= 1; $i --) {
                $sort_path = implode('/', array_slice($path_arr, 0, $i));
                $filter_segments = array_slice($path_arr, $i);
                $filter_sort = $this->model->getSort($sort_path);
                if ($filter_sort && $this->parser->applySelectPathFilters($filter_segments, $filter_sort)) {
                    $path = $sort_path;
    ...
    private function getListPage($sort)
    {
        ...
        if ($sort->listtpl) {
            ...
            $content = $this->parser->parserAfter($content, $sort); // CMS公共标签后置解析
        } else {
