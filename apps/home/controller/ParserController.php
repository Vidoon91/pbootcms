...
class ParserController extends Controller
{
    ...
    // 解析全局后置公共标签
    public function parserAfter($content, $sort = null)
    {
        // 默认页面信息替换
        ...
        $content = str_replace('{pboot:tag}', $this->getCurrentTagText(), $content); // 当前tag关键字
                                                                                     
        // 解析个人扩展标签，升级不覆盖
        $content = $this->parserSelectSeoLabel($content, $sort); // CMS筛选TDK规则解析
        ...
        $content = $this->parserSelectGroupLabel($content, $sort); // CMS筛选分组显示控制
        $content = $this->parserSelectAllLabel($content); // CMS筛选全部标签解析
        $content = $this->parserSelectLabel($content, $sort); // CMS筛选标签解析
    ...
    private function getCurrentTagText()
    {
        $tag = get('tag', 'vars');
        if (! $tag) {
            return '';
        }
        $tag = trim(rawurldecode(html_entity_decode($tag, ENT_QUOTES, 'UTF-8')));
        $tag = str_replace('+', ' ', $tag);
                ...
                switch ($matches[1][$i]) {
                    ...
                    case 'pic':
                        if ($sort->pic) {
							...
							$toppic = $this->model->getSortPic($tcode);
							if($toppic){
							$content = str_replace($matches[0][$i], $this->adjustLabelData($params, SITE_DIR . $toppic), $content);
							}else{
							$content = str_replace($matches[0][$i], $this->adjustLabelData($params, SITE_DIR . '/images/nobanner.jpg'), $content);
							}
                        }
                        break;
    ...
    private function parserSelectSeoLabel($content, $sort = null)
    {
        if (! $sort || ! isset($sort->mcode)) {
            return $content;
        }

        $current = $this->getCurrentSelectRawValues($sort->mcode);
        if (! $current) {
            return $content;
        }

        $model_rules = array();
        if (! empty($sort->select_filter_seo_rules)) {
            $category_rules = json_decode($sort->select_filter_seo_rules, true);
            $model_rules = $this->normalizeSelectSeoModelRules($category_rules);
        }

        if (! $model_rules) {
            $rules = $this->getSelectUrlPathSeoRules();
            $mcode = (string) $sort->mcode;
            if (empty($rules['models'][$mcode]) || ! is_array($rules['models'][$mcode])) {
                return $content;
            }
            $model_rules = $this->normalizeSelectSeoModelRules($rules['models'][$mcode]);
        }

        $replacements = array(
            'title' => array('/<title>[\s\S]*?<\/title>/i', '<title>%s</title>'),
            'keywords' => array('/<meta\s+name=["\']keywords["\'][^>]*>/i', '<meta name="keywords" content="%s">'),
            'description' => array('/<meta\s+name=["\']description["\'][^>]*>/i', '<meta name="description" content="%s">')
        );

        if (! empty($model_rules['default']) && is_array($model_rules['default'])) {
            foreach ($replacements as $key => $replace) {
                if (empty($model_rules['default'][$key])) {
                    continue;
                }
                if ($key === 'keywords') {
                    $value = $this->replaceSelectSeoKeywordPlaceholders($model_rules['default'][$key], $current, $sort);
                } else {
                    $value = $this->replaceSelectSeoPlaceholders($model_rules['default'][$key], $current, $sort);
                }
                $value = htmlspecialchars($value, ENT_QUOTES, 'UTF-8');
                $content = preg_replace($replace[0], sprintf($replace[1], $value), $content, 1);
            }
        }
        ...
        foreach ($model_rules as $rule) {
            ...
            if (! $this->matchSelectSeoRule($rule['match'], $current)) {
                continue;
            }
            ...
            foreach ($replacements as $key => $replace) {
                ...
                if ($key === 'keywords') {
                    $value = $this->replaceSelectSeoKeywordPlaceholders($rule[$key], $current, $sort);
                } else {
                    $value = $this->replaceSelectSeoPlaceholders($rule[$key], $current, $sort);
                }
                $value = htmlspecialchars($value, ENT_QUOTES, 'UTF-8');
                $content = preg_replace($replace[0], sprintf($replace[1], $value), $content, 1);
            }
    ...
    private function normalizeSelectSeoModelRules($rules)
    {
        if (! is_array($rules)) {
            return array();
        }

        if (isset($rules['default']) || isset($rules['rules'])) {
            $result = isset($rules['rules']) && is_array($rules['rules']) ? $rules['rules'] : array();
            if (isset($rules['default']) && is_array($rules['default'])) {
                $result['default'] = $rules['default'];
            }
            return $result;
        }
    ...
    private function matchSelectSeoRule($match, $current)
    {
        foreach ($match as $field => $value) {
            ...
            foreach ($values as $one) {
                $one = $this->normalizeSeoMatchValue($one);
                if ($one === $this->normalizeSeoMatchValue($current[$field]) || $one === $this->normalizeSeoMatchValue($this->buildFilterSlug($current[$field]))) {
                    $matched = true;
                    break;
                }
            }
    ...
    private function replaceSelectSeoKeywordPlaceholders($text, $current, $sort)
    {
        $result = array();
        $keywords = preg_split('/[,\x{FF0C}]/u', (string) $text);
        foreach ($keywords as $keyword) {
            ...
            $show = true;
            if (preg_match_all('/\{filter:([\w\-]+)\}/', $keyword, $matches)) {
            ...
            if ($show) {
                $keyword = $this->replaceSelectSeoPlaceholders($keyword, $current, $sort);
                if ($keyword !== '') {
                    $result[] = $keyword;
                }
            }
    ...
    private function replaceSelectSeoPlaceholders($text, $current, $sort)
    {
        foreach ($current as $field => $value) {
            $text = str_replace('{filter:' . $field . '}', $value, $text);
        }
        $text = preg_replace('/\{filter:[\w\-]+\}/', '', $text);

        $filter_text = implode(' ', array_values($current));
        $text = str_replace('{filters}', $filter_text, $text);

        foreach (array('name', 'subname', 'title') as $field) {
            if (isset($sort->{$field})) {
                $text = str_replace('{sort:' . $field . '}', $sort->{$field}, $text);
            }
        }
    ...
    private function normalizeSeoMatchValue($value)
    {
        $value = trim(rawurldecode(html_entity_decode((string) $value, ENT_QUOTES, 'UTF-8')));
        $value = str_replace('+', ' ', $value);
        return function_exists('mb_strtolower') ? mb_strtolower($value, 'UTF-8') : strtolower($value);
    }

    private function getCurrentSelectRawValues($mcode = '')
    {
        $values = array();
        $fields = $mcode !== '' ? $this->getSelectPathFields($mcode) : $this->getAllSelectPathFields();
        foreach ($fields as $field) {
            if (! ! $value = get($field, 'vars')) {
                $values[$field] = $this->decodeSelectFilterValue($value);
            }
        }
        return $values;
    }

    // Render or suppress a complete filter group for the current category.
    public function parserSelectGroupLabel($content, $sort = null)
    {
        $pattern = '/\{pboot:selectgroup(\s+[^}]+)?\}([\s\S]*?)\{\/pboot:selectgroup\}/';
        if (! preg_match_all($pattern, $content, $matches)) {
            return $content;
        }
        ...
        for ($i = 0; $i < $count; $i ++) {
            ...
            $mcode = isset($params['mcode']) ? $params['mcode'] : ($sort && isset($sort->mcode) ? $sort->mcode : '');
            $scode = isset($params['scode']) ? $params['scode'] : ($sort && isset($sort->scode) ? $sort->scode : '');
            ...
            $data = $this->model->getSelect(escape_string($field), escape_string($mcode), escape_string($scode));
            $content = str_replace($matches[0][$i], $data ? $matches[2][$i] : '', $content);
        }
        return $content;
    }
    ...
    public function parserSelectAllLabel($content)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if ($this->selectPathUrlEnabled() && in_array($field, $this->getAllSelectPathFields(), true) && $path = $this->buildSelectPathUrlFromCurrent($field, null)) {
                    if (get($field, 'vars')) {
    ...
    // 解析筛选标签
    public function parserSelectLabel($content, $sort = null)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                $params = $this->parserParam($matches[1][$i]);
                $field = '';
                $mcode = $sort && isset($sort->mcode) ? $sort->mcode : '';
                $scode = $sort && isset($sort->scode) ? $sort->scode : '';
                ...
                // 读取数据
                if (! ! $data = $this->model->getSelect(escape_string($field), escape_string($mcode), escape_string($scode))) {
                    $data = explode(',', $data);
                } else {
                    $data = array();
                }
                    ...
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        ...
                        switch ($matches2[1][$j]) {
                            ...
                            case 'current':
                                ...
                                break;
                            case 'active':
                                $active_class = isset($params['class']) ? $params['class'] : 'active';
                                $active_class = $this->isSelectCurrent($value, get($field, 'vars')) ? $active_class : '';
                                $one_html = str_replace($matches2[0][$j], $active_class, $one_html);
                                break;
                            case 'link':
                                if ($this->selectPathUrlEnabled() && $sort && in_array($field, $this->getSelectPathFields($mcode), true)) {
                                    $selected = $this->getCurrentSelectValues($mcode);
                                    $selected[$field] = $this->buildFilterSlug($value);
                                    $base_link = $this->parserLink($sort->type, $sort->urlname, 'list', $sort->scode, $sort->filename, '', '');
                                    $link = $this->buildSelectPathUrl($base_link, $selected, $mcode);
                                } else {
                                    $qs = $output; // 需使用中间变量，避免多个链接相同问题
                                    $qs[$field] = $this->buildFilterSlug($value);
    ...
    // 解析内容列表标签
    private function isSelectCurrent($value, $current)
    {
        $normalize = function ($data) {
            $data = urldecode(html_entity_decode((string) $data, ENT_QUOTES, 'UTF-8'));
        ...
        $value_slug = $this->buildFilterSlug($value);
        $current_values = array_map($normalize, explode(',', $current));
        foreach ($current_values as $current_value) {
            if ($value === $current_value || $value_slug === $current_value) {
                return true;
            }
            if ($value === $this->decodeSelectFilterValue($current_value)) {
                return true;
            }
        }
        return false;
    ...
    public function selectPathUrlEnabled()
    {
        // Path filtering only applies to configured model fields, so no extra master switch is needed.
        return true;
    }

    public function applySelectPathFilters($segments, $sort = null)
    {
        if (! $this->selectPathUrlEnabled() || ! $segments) {
            return false;
        }

        $url_break_char = $this->config('url_break_char') ?: '_';
        $last_key = count($segments) - 1;
        if (isset($segments[$last_key]) && preg_match('/^(.+)' . preg_quote($url_break_char, '/') . '([0-9]+)$/', $segments[$last_key], $matches)) {
            $segments[$last_key] = $matches[1];
            $_GET['page'] = $matches[2];
        ...
        foreach ($segments as $segment) {
            ...
            $filter = $this->resolveSelectPathSegment($segment, $sort, $used_fields);
            if (! $filter) {
                return false;
            }
            $filters[$filter['field']] = $filter['value'];
            $used_fields[] = $filter['field'];
        }
    ...
    protected function buildFilterSlug($value)
    {
    ...
    private function buildLegacyFilterSlug($value)
    {
        $value = trim(html_entity_decode((string) $value, ENT_QUOTES, 'UTF-8'));
        if ($value === '') {
            return '';
    ...
    protected function normalizeSelectFilterValue($value)
    {
        return escape_string($this->decodeSelectFilterValue($value));
    }

    private function decodeSelectFilterValue($value)
    {
        $value = trim(rawurldecode(html_entity_decode((string) $value, ENT_QUOTES, 'UTF-8')));
        $value = str_replace('+', ' ', $value);
        $lower_value = function_exists('mb_strtolower') ? mb_strtolower($value, 'UTF-8') : strtolower($value);

        $reverse_map = array_flip($this->getFilterSlugMap());
        if (isset($reverse_map[$lower_value])) {
            return $reverse_map[$lower_value];
        }
    ...
    private function getSelectFilterFieldBySlug($slug, $mcode = '')
    {
        $slug = $this->normalizeFilterSlug($slug);
        $map = $this->getFilterSlugFieldMap($mcode);
        if (isset($map[$slug])) {
            return $map[$slug];
        }
    ...
    private function resolveSelectPathSegment($slug, $sort = null, $used_fields = array())
    {
        $slug = $this->normalizeFilterSlug($slug);
        if ($slug === '') {
            return false;
        ...
        $mcode = $sort && isset($sort->mcode) ? $sort->mcode : '';
        $scode = $sort && isset($sort->scode) ? $sort->scode : '';
        $mapped_field = $this->getSelectFilterFieldBySlug($slug, $mcode);
        $fields = $mapped_field ? array($mapped_field) : $this->getSelectPathFields($mcode);

        foreach ($fields as $field) {
            if (! $field || in_array($field, $used_fields, true)) {
                continue;
            }

            foreach ($this->getSelectCandidateValues($field, $mcode, $scode) as $value) {
                if ($value !== '' && ($this->normalizeFilterSlug($this->buildFilterSlug($value)) === $slug || $this->normalizeFilterSlug($this->buildLegacyFilterSlug($value)) === $slug)) {
                    return array(
                        'field' => $field,
                        'value' => $value
                    );
            ...
            if ($mapped_field === $field) {
                return array(
                    'field' => $field,
                    'value' => $this->decodeSelectFilterValue($slug)
                );
            }
        }
    ...
    private function getSelectCandidateValues($field, $mcode = '', $scode = '')
    {
        $values = array();
        $data_list = array(
            $this->model->getSelect(escape_string($field), escape_string($mcode), escape_string($scode))
        );
    ...
    private function getCurrentSelectValues($mcode = '')
    {
        $values = array();
        $fields = $mcode !== '' ? $this->getSelectPathFields($mcode) : $this->getAllSelectPathFields();
        foreach ($fields as $field) {
            if (! ! $value = get($field, 'vars')) {
                $values[$field] = $this->buildFilterSlug($this->decodeSelectFilterValue($value));
            }
        }
        return $values;
    }

    private function buildSelectPathUrlFromCurrent($remove_field, $set_slug)
    {
        $base_link = $this->getCurrentSelectPathBase();
        if (! $base_link) {
            return '';
        }

        $selected = $this->getCurrentSelectValues();
        if ($set_slug === null) {
            unset($selected[$remove_field]);
        } else {
            $selected[$remove_field] = $set_slug;
        }
        return $this->buildSelectPathUrl($base_link, $selected);
    }

    private function getCurrentSelectPathBase()
    {
        $request_uri = isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : URL;
        $path = parse_url($request_uri, PHP_URL_PATH);
        $path = trim($path, '/');

        if (SITE_DIR && strpos($path, trim(SITE_DIR, '/')) === 0) {
            $path = trim(substr($path, strlen(trim(SITE_DIR, '/'))), '/');
        }

        $segments = $path ? explode('/', $path) : array();
        $current_slugs = array_values($this->getCurrentSelectValues());
        while ($segments) {
            $last = end($segments);
            $url_break_char = $this->config('url_break_char') ?: '_';
            if (preg_match('/^(.+)' . preg_quote($url_break_char, '/') . '[0-9]+$/', $last, $matches)) {
                $last = $matches[1];
            }
            $last_slug = $this->normalizeFilterSlug($last);
            if (! in_array($last_slug, $current_slugs, true) && ! $this->getSelectFilterFieldBySlug($last_slug)) {
                break;
            }
            array_pop($segments);
        }
        ...
        if ($segments) {
            return SITE_INDEX_DIR . '/' . implode('/', $segments) . '/';
        }

        if (isset($_SERVER["QUERY_STRING"]) && ! ! $qs = $_SERVER["QUERY_STRING"]) {
            parse_str($qs, $output);
            if ($output && ! current($output)) {
                return SITE_INDEX_DIR . '/?' . trim(key($output), '/') . '/';
            }
        }
    ...
    private function buildSelectPathUrl($base_link, $selected, $mcode = '')
    {
    ...
    private function getSelectPathFields($mcode = '')
    {
        $rules = $this->getSelectUrlPathRules();
        $mcode = (string) $mcode;
        if ($mcode !== '' && isset($rules['models'][$mcode]) && is_array($rules['models'][$mcode])) {
            return $this->normalizeSelectPathFields($rules['models'][$mcode]);
        }
        return array();
    }

    private function getAllSelectPathFields()
    {
        $rules = $this->getSelectUrlPathRules();
        $fields = array();
        if (isset($rules['models']) && is_array($rules['models'])) {
            foreach ($rules['models'] as $model_fields) {
                if (is_array($model_fields)) {
                    $fields = array_merge($fields, $model_fields);
                }
            }
        }
        return $this->normalizeSelectPathFields($fields);
    }

    private function normalizeSelectPathFields($fields)
    {
        $result = array();
        foreach ($fields as $field) {
    ...
    private function getFilterSlugFieldMap($mcode = '')
    {
        $map = array();
        $rules = $this->getSelectUrlPathRules();
        $allowed_fields = $mcode !== '' ? $this->getSelectPathFields($mcode) : array();
        if (isset($rules['options']) && is_array($rules['options'])) {
            foreach ($rules['options'] as $field => $options) {
                if ($allowed_fields && ! in_array($field, $allowed_fields, true)) {
                    continue;
                }
                if (! is_array($options)) {
                    continue;
                ...
                foreach ($options as $slug) {
                    if ($slug !== '') {
                        $slug = function_exists('mb_strtolower') ? mb_strtolower($slug, 'UTF-8') : strtolower($slug);
                        $map[$this->normalizeFilterSlug($slug)] = $field;
                    }
                }
            }
        }
    ...
    private function normalizeFilterSlug($slug)
    {
        $slug = trim(rawurldecode((string) $slug));
        return function_exists('mb_strtolower') ? mb_strtolower($slug, 'UTF-8') : strtolower($slug);
    }

    private function getSelectUrlPathRules()
    {
        static $rules = null;
        if ($rules !== null) {
            return $rules;
        ...
        $config = trim((string) $this->config('select_url_path_rules'));
        $config = html_entity_decode(stripcslashes($config), ENT_QUOTES, 'UTF-8');
        if ($config) {
    ...
    private function getSelectUrlPathSeoRules()
    {
        static $rules = null;
        if ($rules !== null) {
            return $rules;
        ...
        $config = trim((string) $this->config('select_url_path_seo_rules'));
        $config = html_entity_decode(stripcslashes($config), ENT_QUOTES, 'UTF-8');
        if ($config) {
            $json = json_decode($config, true);
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if ($page) {
                    ...
                    if (! ! $get_tag = get('tag', 'vars')) {
                        $get_tag = $this->normalizeTagFilterValue($get_tag);
                        if ($fuzzy) {
                    ...
                    foreach ($_GET as $key => $value) {
                        if (preg_match('/^ext_[\w\-]+$/', $key)) { // 其他字段不加入
                            $where3[$key] = $this->normalizeSelectFilterValue(get($key, 'vars'));
                        }
                    }
                }
    ...
    public function parserCurrentContentLabel($content, $sort, $data)
    {
        ...
        if ($this->config('tpl_html_cache')) {
            if (! isset($this->var['addvisits'])) {
                $visits = "<script src='" . Url::home('Do/visits/id/' . $data->id, false) . "' async='async'></script>";
    ...
    public function parserContentPicsLabel($content)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                // 跳过未指定id或scode的列表
                if (! array_key_exists('id', $params) && ! array_key_exists('scode', $params)) {
                    continue;
                }
                ...
                if ($id == - 1 && $scode != - 1) {
                    $about = $this->model->getAbout(escape_string($scode));
                    $id = $about ? $about->id : - 1;
                }
                    ...
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        ...
                        switch ($matches2[1][$j]) {
                            ...
                            case 'i':
                                ...
                                break;
                            case 'text':
                                $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params, trim($value['tags'])), $one_html);
                                break;
                            case 'link':
                                $url_rule_type = $this->config('url_rule_type') ?: 3;
                                $tag_slug = $this->buildTagSlug($value['tags']);
                                if ($target == 'tag') {
                                    if ($url_rule_type == 3) {
                                        $link = Url::home('tag=' . rawurlencode($tag_slug), false);
                                    } else {
                                        $link = Url::home('tag/' . rawurlencode($tag_slug), false);
                                    }
                                } else {
                                    ...
                                    if ($url_rule_type == 3) {
                                        $link = $link . '&tag=' . rawurlencode($tag_slug);
                                    } else {
                                        $link = $link . '?tag=' . rawurlencode($tag_slug);
                                    }
                                }
    ...
    // 生成更适合 SEO 的 tag URL slug，例如 Center Anchor Insulator -> center-anchor-insulator
    protected function buildTagSlug($tag)
    {
        $tag = trim(html_entity_decode($tag, ENT_QUOTES, 'UTF-8'));
        $slug = preg_replace('/[^\p{L}\p{N}]+/u', '-', $tag);
        $slug = trim($slug, '-');
        $slug = function_exists('mb_strtolower') ? mb_strtolower($slug, 'UTF-8') : strtolower($slug);
        return $slug ?: $tag;
    }

    // tag URL 使用短横线，筛选数据库时还原为空格以匹配后台原始 tag
    protected function normalizeTagFilterValue($tag)
    {
        $tag = trim(rawurldecode(html_entity_decode($tag, ENT_QUOTES, 'UTF-8')));
        $tag = str_replace('+', ' ', $tag);
        if (strpos($tag, '-') !== false && strpos($tag, ' ') === false) {
        ...
        switch ($label) {
            ...
            case 'headpic':
                if ($data->headpic) {
                    ...
                    $content = str_replace($search, SITE_DIR . '/skin/images/default-avatar.svg', $content);
                }
                break;
            case 'pheadpic':
                if ($data->pheadpic) {
            ...
            case 'homepage':
                if (! empty($data->uid)) {
                    $content = str_replace($search, Url::home('member/home', null, 'id=' . $data->uid), $content);
                } else {
                    $content = str_replace($search, 'javascript:void(0);', $content);
                }
                break;
            case 'replyaction':
                if ($data->pid) {
    ...
    public function parserIfLabel($content)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                //if标签解析
				$matches[1][$i] = str_ireplace(array('.','\\x','//','%2F%2F','/*','/%2A','%2F%2A','%2F*','%28','%29','(',')',';','%3B','%60','`'),'',$matches[1][$i]);
...
//        var_dump(111);die;
        ...
        switch ($label) {
            ...
            case 'enclosuresize':
                if ($data->enclosure && file_exists(ROOT_PATH . $data->enclosure)) {
                    $content = str_replace($search, $this->adjustLabelData($params, $this->formatFileSize(filesize(ROOT_PATH . $data->enclosure))), $content);
                } else {
                    $content = str_replace($search, 0, $content);
                }
                break;
            case 'enclosuretype':
                $content = str_replace($search, $this->adjustLabelData($params, $this->getEnclosureType($data->enclosure)), $content);
                break;
            case 'enclosureicon':
                $content = str_replace($search, $this->adjustLabelData($params, $this->getEnclosureIcon($data->enclosure)), $content);
                break;
            case 'likeslink':
    ...
    // Format attachment file size for enclosure size tags.
    ...
    // Get readable attachment type by file extension.
    protected function getEnclosureType($path)
    {
        $ext = $this->getEnclosureExt($path);
        $types = array(
            'pdf' => 'PDF',
            'doc' => 'Word',
            'docx' => 'Word',
        ...
        return isset($types[$ext]) ? $types[$ext] : 'Document';
    }

    // Get Font Awesome file icon suffix by file extension.
    protected function getEnclosureIcon($path)
    {
        $ext = $this->getEnclosureExt($path);
        $icons = array(
            'pdf' => 'pdf',
        ...
        return isset($icons[$ext]) ? $icons[$ext] : 'alt';
    }

    protected function getEnclosureExt($path)
    {
