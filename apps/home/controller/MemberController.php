...
class MemberController extends Controller
{
    ...
    // 成功提示后延迟跳转
    private function alertDelayLocation($info, $url, $seconds = 5)
    {
        $info_json = json_encode(strip_tags($info), JSON_UNESCAPED_UNICODE);
        $url_json = json_encode($url, JSON_UNESCAPED_UNICODE);
...
<head>
    ...
    <title>操作成功</title>
    <style>
        body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Noto Sans SC",Arial,sans-serif;background:#f8fafc;color:#0f172a;}
        .notice{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;box-sizing:border-box;}
        .card{width:100%;max-width:420px;background:#fff;border:1px solid #e2e8f0;border-radius:14px;box-shadow:0 16px 36px rgba(15,23,42,.08);padding:28px;text-align:center;}
        .title{font-size:20px;font-weight:700;margin:0 0 10px;}
        .text{font-size:15px;color:#475569;line-height:1.7;margin:0;}
        .link{display:inline-flex;margin-top:18px;color:#1a56db;text-decoration:none;font-weight:600;}
    </style>
</head>
<body>
    <div class="notice">
        <div class="card">
            ...
            <p class="text"><span id="countdown"></span> 秒后自动返回评论页面。</p>
            <a class="link" id="jumpLink" href="#">立即返回</a>
        </div>
    </div>
    <script>
        var message = {$info_json};
        var target = {$url_json};
        var seconds = {$seconds};
        document.getElementById('noticeTitle').textContent = message;
        document.getElementById('jumpLink').href = target;
...
HTML;
        ...
        if ($_POST) {
            ...
            if (! ! $login = $this->model->login("(username='$username' or useremail='$username' or usermobile='$username') AND password='$password'")) {
                ...
                if (! ! $backurl = get('backurl')) {
                    $this->alertDelayLocation('登录成功！', $backurl, 5);
                } else {
    ...
    public function register()
    {
        ...
        if ($_POST) {
            ...
            if (! preg_match('/^1[0-9]{10}$/', $usermobile)) {
                alert_back('手机号格式不正确，请输入正确的手机号码！');
            }
            if ($this->model->checkUsername("username='$username' OR useremail='$username' OR usermobile='$username'")) {
                alert_back('您输入的用户名已被注册或已被占用！');
            }
            ...
            // 执行注册
            if ($insert_id = $this->model->registerGetId($data)) {
                session('lastreg', time()); // 记录最后提交时间
                if ($status) {
                    $login = $this->model->getUserById($insert_id);
                    session('pboot_uid', $login->id);
                    session('pboot_ucode', $login->ucode);
                    session('pboot_username', $login->username);
                    ...
                    session('pboot_gname', $login->gname);
                    if (! ! $backurl = get('backurl')) {
                        $this->alertDelayLocation('注册成功，已自动登录！', $backurl, 5);
                    } else {
                        alert_location('注册成功，已自动登录！', Url::home('member/ucenter'), 1);
                    }
                } else {
    ...
    public function home()
    {
        ...
        if ($headpic) {
            if (! preg_match('/^http/', $headpic)) {
                $headpic = SITE_DIR . $headpic;
            }
        } else {
            $headpic = SITE_DIR . '/skin/images/default-avatar.svg';
        }

        $nickname = $user->nickname ?: ($user->username ?: '会员');
        $content = parent::parser($this->htmldir . 'member/home.html');
        $content = $this->parser->parserBefore($content);
        $content = str_replace('{pboot:pagetitle}', $nickname . '-会员主页-{pboot:sitetitle}-{pboot:sitesubtitle}', $content);
        $content = str_replace('{member:id}', $user->id, $content);
        $content = str_replace('{member:ucode}', $user->ucode, $content);
        $content = str_replace('{member:nickname}', $nickname, $content);
        $content = str_replace('{member:headpic}', $headpic, $content);
        $content = str_replace('{member:gname}', $user->gname ?: '普通会员', $content);
        $content = str_replace('{member:registertime}', $user->register_time, $content);
        $content = str_replace('{member:logincount}', $user->login_count, $content);
        $content = $this->parser->parserPositionLabel($content, 0, '会员主页', Url::home('member/home', null, 'id=' . $id));
        $content = $this->parser->parserSpecialPageSortLabel($content, - 4, '会员主页', Url::home('member/home', null, 'id=' . $id));
