...
class CmsModel extends Model
{
    ...
    public function getLists($acode, $scode, $num, $order, $filter = array(), $tags = array(), $select = array(), $fuzzy = true)
    {
        ...
        return $this->attachExtDataList($result);
    }
    ...
    public function getContent($acode, $id)
    {
        ...
        $result = $this->attachExtDataRow($result);
        if ($result) {
            $data2 = array(
                'visits' => '+=1'
            );
    ...
    private function getExtDataByContentId($id)
    {
        $table = $this->getExtTableByContentId($id);
        return parent::table($table)->where("contentid='$id'")->decode()->find();
    }

    private function attachExtDataRow($row)
    {
        if (! $row) {
            return $row;
        }
        $ext = $this->getExtDataByContentId($row->id);
        if ($ext) {
            foreach ($ext as $k => $v) {
                $row->$k = $v;
            }
    ...
    private function attachExtDataList($list)
    {
        if (! $list) {
            return $list;
        }
        foreach ($list as $item) {
            $this->attachExtDataRow($item);
        }
        return $list;
    }
}
