...
//拦截前端当前链接的字符
function forbWords($thisurl){
	$testwords = array('x3c','x3e','%27','%28','%29','/*','2F2A','/%2A','%2F%2A','%2F*','\\x',';','%3B','%3C','%3E','(',')','<','>','%60','`','%22','"',"'");
	foreach($testwords as $value){
		if(stripos($thisurl,$value) !== false){
			_404('您访问的内容不存在，请核对后重试！');
			break;
		}
	}
}
function forbpost($thisurl){
	$testwords = array('select','insert','update','delete','where');
	foreach($testwords as $value){
		if(stripos($thisurl,$value) !== false){
			_404('您访问的内容不存在，请核对后重试！');
			break;
